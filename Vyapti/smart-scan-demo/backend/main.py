...
def get_emitters() -> List[Dict[str, Any]]:
    """
    Returns ALL radar emitters extracted from all TSRD configs (folder 3 +
    config_0).  Tries the merged tsrd_all_emitters.json first (generated by
    extract_folder3_data.py), then falls back to the original 72-emitter
    JSON so the endpoint never returns an empty list.
    """
    import json
    from pathlib import Path
    lib_dir = Path(__file__).resolve().parent.parent / "lib"
    # Prefer merged all-emitters file
...
def get_dataset_configs() -> Dict[str, Any]:
    """
    Returns metadata for all loaded TSRD scenario configs.
    Uses the pre-processed tsrd_folder3_configs.json generated by
    extract_folder3_data.py (9 configs total: config_0 + 8 from folder 3).
    """
    import json
    from pathlib import Path
    from dataset import is_dataset_loaded, dataset_size, dataset_config_stats

    json_path = Path(__file__).resolve().parent.parent / "lib" / "tsrd_folder3_configs.json"
    configs_meta = []
...
def get_environment_matrix(
    ...
    Added: config_id query param selects which TSRD config's obs matrix to use.
    Supports config_0 (original) or any folder-3 config (config_1, config_106, ...).
    """
    from pathlib import Path
    import numpy as np
    ...
    if config_id == "config_0":
        ...
        # folder-3 pre-processed obs matrix
